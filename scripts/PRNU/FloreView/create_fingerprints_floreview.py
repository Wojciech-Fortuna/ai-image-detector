...
def _is_flat_subpath(root_path: Path, group_dir: Path) -> bool:
    try:
        rel_parts = root_path.relative_to(group_dir).parts
    except ValueError:
        return False
    return any("flat" in p.lower() for p in rel_parts)


def collect_flat_images(group_dir: Path) -> List[Path]:
    imgs: List[Path] = []
    for root, _, files in os.walk(group_dir):
        root_path = Path(root)
        if not _is_flat_subpath(root_path, group_dir):
            continue
        for f in files:
            if f.lower().endswith(VALID_EXT):
                imgs.append(root_path / f)
    return sorted(imgs)
...
def group_paths_by_native_hw(paths: List[str]) -> Dict[Tuple[int, int], List[str]]:
    ...
    return dict(sorted(buckets.items(), key=lambda kv: (kv[0][0], kv[0][1])))
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "FloreView PRNU builder (folders) — fingerprint logic IDENTICAL to create_fingerprints_dresden.py\n"
            "Reference images: ALL images under group subtree containing 'Flat' in the path.\n"
            "Grouping by native HxW; crops are square sizes; CENTER CROP."
        )
    )
    ap.add_argument("data_root", type=Path, nargs="?", default=Path("FloreView_Dataset"))
    ap.add_argument("--results", type=Path, default=Path("Results_FloreView_Paper7701_DIDN"))
    ap.add_argument("--resume", action="store_true")
    ap.add_argument("--root", action="store_true")
    ap.add_argument("--dry-run", action="store_true")

    ap.add_argument("--sizes", type=str, default="128,256,512",
    ...
    ap.add_argument("--residual", type=str, default="didn",
                    ...
                    help="Residual extractor: didn, gauss, wavelet.")
    ...
    if args.dry_run:
        print("[DRY RUN] Groups:")
        for name in order:
            gdir = groups[name]
            total = dir_size_bytes(gdir)
            nimg = count_images(gdir)
            print(f"  - {name:30s} {bytes_to_human(total)} ({nimg} images total; PRNU only from Flat)")
        print(f"[DRY RUN] residual={args.residual} save_baseline={'YES' if args.save_baseline else 'NO'} sizes={sizes} bases={bases}")
        return

    already_done = set(state.get("done", []))
    to_process = [g for g in order if g not in already_done]
    if not to_process:
        print("Nothing to do. All groups are already processed.")
        return

    print(f"Found {len(order)} groups; to process {len(to_process)}...")
    print(f"[INFO] residual={args.residual} save_baseline={'YES' if args.save_baseline else 'NO'}")
    print(f"[INFO] crops --sizes={sizes} (CENTER CROP, square)")
    print(f"[INFO] output naming: <group>__<HxW>__<size>__...npz")

    for group in to_process:
        gdir = groups[group]
        total_bytes = dir_size_bytes(gdir)
        img_count = count_images(gdir)
        print(f"\n==> Group: {group} ({bytes_to_human(total_bytes)}, {img_count} images total)")
        print("    [INFO] Using only images under subtrees containing 'Flat'.")

        flat_imgs = collect_flat_images(gdir)
        if not flat_imgs:
            print("[SKIP] No Flat images.")
            already_done.add(group)
            save_state(state_path, {"done": sorted(list(already_done))})
            continue

        ref_paths_all = [str(p.resolve()) for p in flat_imgs]

        buckets = group_paths_by_native_hw(ref_paths_all)
        if not buckets:
