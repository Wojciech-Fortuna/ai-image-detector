...
def load_train_paths_for_group(group_name: str, data_root: Path, splits_root: Path) -> List[str]:
...
def parse_square_sizes(s: str) -> List[int]:
    ...
    for raw in (s or "").split(","):
        ...
        if not tok.isdigit():
            raise ValueError(
                f"Invalid --sizes token: '{raw}' (expected comma-separated ints, e.g. 128,256,512)"
            )
        val = int(tok)
        if val <= 0:
...
def noise_extract_wavelet(
    ...
    den = den[: img.shape[0], : img.shape[1]]  # czasem 1px różnicy
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "PRNU builder (folders) + paper7701 HF enhancement; logic aligned with create_fingerprints_dresden.py.\n"
            "Reference images ALWAYS from <splits>/<group>/train.txt (relative to data_root).\n"
            "If group has mixed native sizes, fingerprints are computed separately per native HxW bucket.\n"
            "Crop sizes: --sizes is comma-separated INTS => square CENTER crops."
        )
    )
    ap.add_argument("data_root", type=Path, nargs="?", default=Path("."),
                    help="Katalog z danymi (odpowiednik work_dir z pierwszego skryptu)")
    ap.add_argument("--splits", type=Path, required=True,
                    help="Folder z podziałem (Results_Splits_...). Używamy <splits>/<group>/train.txt.")

    ap.add_argument("--results", type=Path, default=Path("Results_Paper7701_DIDN"))
    ...
    if args.dry_run:
        ...
        for name in order:
            ...
            print(f"  - {name:30s} {bytes_to_human(total)} ({nimg} imgs) train.txt={'OK' if train_file.exists() else 'MISSING'}")
        return

    already_done = set(state.get("done", []))
    to_process = [g for g in order if g not in already_done]
    if not to_process:
        print("Nothing to do. All groups done.")
        return
    ...
    for group in to_process:
        gdir = groups[group]
        print(f"\n==> Group: {group} ({count_images(gdir)} images on disk)")

        ref_paths = load_train_paths_for_group(group, data_root=data_root, splits_root=splits_root)
        if not ref_paths:
